...
def parse_from_path(
    ...
    for split_dir in split_dirs:
        ...
        for file in files:
            in_file_path = f"{split_dir}/{file}"
            out_file_path = f"{out_path}/{split_dir.split('/')[-1]}/{file.split('.')[0]}.parquet"
            ...
            print(f"Saved parsed file at {out_file_path}. Elapsed {(end - start) / 60} minutes")
